...
def test_summary_script_writes_all_expected_csvs(tmp_path: Path) -> None:
    ...
    _write_json(
        stage_dir / "stage_manifest.json",
        {
            ...
            "mtl": {
                ...
                "balanced_best": {"metrics_dir": str(balanced_metrics_dir)},
            },
        },
    )
    _write_json(
    ...
    _write_json(
        best_metrics_dir / "eval_results_test.json",
        {
            "results": {
    ...
    _write_json(
        balanced_metrics_dir / "eval_results_test.json",
        {
            "results": {
    ...
    assert {"best", "added_task_best", "balanced_best"} <= {row["checkpoint_view"] for row in rows}
