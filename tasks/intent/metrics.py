"""Metric computation utilities for intent classification."""
...
def _extract_candidate_span(text: str) -> str:
    """Return the portion of `text` that most likely contains the predicted intent."""
    if not text:
        return ""

    match = re.search(r"intent\s*:\s*(.+)", text, flags=re.IGNORECASE | re.DOTALL)
...
def compute_intent_metrics(
    ...
    *,
    ...
    """Compute accuracy and macro-F1 for generated intent labels.

    Args:
        eval_pred: Tuple of (predictions, labels)
        processor: Model processor containing tokenizer
        label_names: List of intent label names
    ...
    for text in pred_texts_raw:
        text = text.strip()
        # Extract intent from chat response format if needed
        if "assistant\n" in text:
...
__all__ = ["compute_intent_metrics"]
