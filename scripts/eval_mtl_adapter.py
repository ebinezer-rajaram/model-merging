...
def main() -> None:
    ...
    for i, task in enumerate(tasks, 1):
        print(f"\n[{i}/{len(tasks)}] Evaluating '{task}' on {split} split...")
        try:
            ...
            setup = prepare_task_for_evaluation(task, processor, split=split, config=task_cfg)
            ...
            maybe_add_interference_delta(task, results[task], split, show_summary=True, eval_tag=None)
